...
import 'package:pfe/components/text_widget.dart';
...
class _ArticlePageState extends State<ArticlePage> {
  var articles = <ToxicityInfo>[];
  var allarticles = <ToxicityInfo>[];
  ...
  _initData() async {
    await CallApi().getPublicData("recommendedarticles").then((response) {
      setState(() {
        ...
        articles = list.map((model) => ToxicityInfo.fromJson(model)).toList();
      });
    });
    await CallApi().getPublicData("allarticles").then((response) {
      setState(() {
        Iterable list = json.decode(response.body);
        allarticles =
            list.map((model) => ToxicityInfo.fromJson(model)).toList();
      });
    });
  }
